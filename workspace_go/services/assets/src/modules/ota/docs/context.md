...
| Reconciler | Engine that dispatches the next paced batch of actionable executions to online devices between the two timers | ReconcilerTimers (drives the start/close timers + the global scan) |
...
| Plan/firmware timers | `ota.timer.start` / `ota.timer.close` / `ota.timer.scan` / `ota.timer.abandon` (STATIC; id in payload + MsgId dedup) | scheduled self-publish (native NATS `@at`) | this module's timers consumer |
...
- NATS consumer on `ota.timer.>` → `ReconcilerTimers` + firmware abandon (start/close/scan/abandon)
...
- `ports.OTASchedulerPort` / `ports.FirmwareSchedulerPort` — native NATS `@at` scheduling (`infrastructure/messaging/nats`)
