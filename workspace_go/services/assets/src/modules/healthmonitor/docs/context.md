...
| Edge presence — connect/disconnect | `${env}.mapexos.presence.advisory` (stream `MAPEXOS-EDGE-PRESENCE` on CORE, queue group `assets-edge-presence`) | `PresenceAdvisory{Event, Protocol, OrgID, AssetUUID, ClientID, SourceIP, Timestamp, ReasonCode, ReasonText}` (cross-service contract `packages/contracts/services/assets/healthmonitor/presence.go`) | Every edge server publishes here on each CONNECT/DISCONNECT — the mapex-mqtt-broker plugin (MQTT) and the mapexLNS Gateway Server (LoRaWAN gateways). One subject; the consumer dispatches by the Event field. |
...
- NATS consumer on `${env}.mapexos.presence.advisory` via stream `MAPEXOS-EDGE-PRESENCE` (durable `{service}-edge-presence`, queue group `assets-edge-presence`, DLQ) — dispatches connect/disconnect by the Event field
- NATS consumer on `mapexos.healthmonitor.scan` (queue-group load-balanced across pods, DLQ)
- `PresencePort` — internal address-driven peer of the presence consumer; not consumed by any other module
...
- **MQTT presence**: MQTT-protocol assets in `heartbeatMode='explicit'` signal liveness via the mapex-mqtt-broker plugin — the broker publishes `${env}.mapexos.presence.advisory` with `Event=connect` on CONNECT and `Event=disconnect` on DISCONNECT. No firmware change required beyond keeping the MQTT connection open.
