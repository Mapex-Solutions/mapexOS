...
// CacheKeyBuilderPort abstracts Redis key construction for the assets
// module's application cache. It exposes ONLY the key-formatting helpers
// the application layer needs, without leaking Redis-specific constants
// (key prefixes) into the application package.
//
...
type CacheKeyBuilderPort interface {
	...
	// per-org asset counter (total assets scoped to orgId).
	//
	// Parameters:
