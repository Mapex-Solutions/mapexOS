...
// configuration from main.go: Fiber at P0, MongoDB/Redis/NATS Core at P5.
...
func TestShutdownIntegration_MirrorsMainConfig(t *testing.T) {
	...
	// P5: NATS Core — simulate close (10ms)
	sm.RegisterFunc("nats-core", 5, func(_ context.Context) error {
		tracker.record("nats-core", 10*time.Millisecond)
		return nil
	})
	...
	for _, expected := range []string{"mongodb", "redis", "nats-core"} {
		if !p5[expected] {
	...
	for _, name := range []string{"mongodb", "redis", "nats-core"} {
...
// Redis and NATS Core close concurrently (same P5 priority).
func TestShutdownIntegration_P5ConnectionsConcurrent(t *testing.T) {
	...
	sm.RegisterFunc("nats-core", 5, makeP5Hook("nats-core"))
