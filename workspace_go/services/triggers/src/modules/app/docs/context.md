# Bounded Context: App (Triggers Bootstrap)
...
This is NOT a business bounded context — it is the **service bootstrap orchestrator** for the `triggers` service. Its single responsibility is to iterate the module registry declared in `src/shared/configuration/modules/config.go` and invoke each module's lifecycle hooks (`InitRepositories`, `InitServices`, `InitInterfaces`) in the correct order. It carries no domain logic, no ports, no entities, and no state of its own — it only wires the DI container and starts the consumer/HTTP surfaces of the real modules (`triggers`, `events`).
...
| Lifecycle phase | One of the three ordered init passes: Repositories → Services → Interfaces | Service runtime phases (execute, retry, DLQ, etc.) |
...
- Calls lifecycle hooks of the real bounded contexts in this service: **triggers** (CRUD + cache management), **events** (execution dispatcher, batch processing, executor registry).
