package appModule

import (
	"fmt"

	"github.com/gofiber/fiber/v2"

	configMod "mapexIam/src/shared/configuration/modules"

	logger "github.com/Mapex-Solutions/mapexGoKit/microservices/logger"
)

// InitModule initializes all modules in 4 phases:
// 1. InitRepositories - Registers all repositories in DIG container
// 2. InitServices - Registers all services in DIG container
// 3. InitInterfaces - Registers HTTP routes and consumers
// 4. InitListeners - Registers NATS event listeners
func InitModule(c *fiber.App) {

	logger.Info("[MODULE:Bootstrap] Initializing Modules")

	// Show initialization order
	for i, mod := range configMod.Modules {
		logger.Info(fmt.Sprintf("[MODULE:Bootstrap] Module %d: %s", i+1, mod.Name))
	}

	// Phase 1: Initialize all repositories
	logger.Info("[MODULE:Bootstrap] Phase 1: Initializing Repositories")

	for _, mod := range configMod.Modules {
		if !mod.Lazy && mod.InitRepositories != nil {
			logger.Info(fmt.Sprintf("[MODULE:%s] Initializing repositories...", mod.Name))
			mod.InitRepositories()
		}
	}

	// Phase 2: Initialize all services
	logger.Info("[MODULE:Bootstrap] Phase 2: Initializing Services")

	for _, mod := range configMod.Modules {
		if !mod.Lazy && mod.InitServices != nil {
			logger.Info(fmt.Sprintf("[MODULE:%s] Initializing services...", mod.Name))
			mod.InitServices()
		}
	}

	// Phase 3: Initialize all interfaces (HTTP + Consumers)
	logger.Info("[MODULE:Bootstrap] Phase 3: Initializing Interfaces (HTTP Routes & Consumers)")

	for _, mod := range configMod.Modules {
		if !mod.Lazy && mod.InitInterfaces != nil {
			logger.Info(fmt.Sprintf("[MODULE:%s] Initializing interfaces...", mod.Name))
			mod.InitInterfaces()
		}
	}

	// Phase 4: Initialize all listeners (NATS event listeners)
	logger.Info("[MODULE:Bootstrap] Phase 4: Initializing Listeners (NATS Events)")

	for _, mod := range configMod.Modules {
		if !mod.Lazy && mod.InitListeners != nil {
			logger.Info(fmt.Sprintf("[MODULE:%s] Initializing listeners...", mod.Name))
			mod.InitListeners()
		}
	}

	logger.Info("[MODULE:Bootstrap] All modules initialized successfully")
}
