...
func InitSwagger() {
	...
	swagger.Init(swagger.Config{
		...
		Description: "Workflow service. Runs the DAG execution engine over workflow definitions, " +
			"manages instances and their executions, archives finished runs, and resolves plugin " +
			"configuration. This HTTP API manages definitions, instances, executions, and plugins.",
		...
		Tags: []swagger.Tag{
			{Name: "Definitions", Description: "Create, query, and manage workflow definitions — the DAGs that the engine executes."},
			{Name: "Instances", Description: "Configured instances of a definition, plus on-demand execution."},
			{Name: "Executions", Description: "Archived execution history of finished workflow runs."},
			{Name: "Plugins", Description: "Plugin registry and configuration used by workflow nodes."},
			{Name: "Fetch Options", Description: "Dynamic option resolution for plugin/node configuration fields."},
		},
	})
}
