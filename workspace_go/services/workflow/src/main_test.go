...
// configuration from main.go: Fiber at P0, MongoDB/Redis/NATS at P5.
...
func TestShutdownIntegration_MirrorsMainConfig(t *testing.T) {
	...
	// P5: Redis — simulate close (10ms)
	sm.RegisterFunc("redis", 5, func(_ context.Context) error {
		tracker.record("redis", 10*time.Millisecond)
		return nil
	})
	...
	// All 4 hooks must have executed
	if len(order) != 4 {
		t.Fatalf("expected 4 hooks executed, got %d: %v", len(order), order)
	}
	...
	for _, expected := range []string{"mongodb", "redis", "nats"} {
		if !p5[expected] {
	...
	for _, name := range []string{"mongodb", "redis", "nats"} {
...
// Redis and NATS close concurrently (same P5 priority), not sequentially.
func TestShutdownIntegration_P5ConnectionsConcurrent(t *testing.T) {
	...
	sm.RegisterFunc("redis", 5, makeP5Hook("redis"))
	...
	if maxConcurrent.Load() < 3 {
		t.Fatalf("expected all 3 P5 hooks running concurrently, max concurrent was %d", maxConcurrent.Load())
	}
}

// TestShutdownIntegration_MongoErrorDoesNotBlockRedisNATS verifies that
// if MongoDB Close fails, Redis and NATS still close successfully.
func TestShutdownIntegration_MongoErrorDoesNotBlockRedisNATS(t *testing.T) {
	sm := shutdown.New()
...
func TestShutdownIntegration_CompletesWithinTimeout(t *testing.T) {
	...
	// Signal arrives at ~50ms, fiber ~10ms, then P5 ~20ms (concurrent)
	// Total should be well under 1s
...
func TestShutdownIntegration_SlowFiberDrainsBeforeConnectionClose(t *testing.T) {
	...
	sm.RegisterFunc("redis", 5, func(_ context.Context) error {
		tracker.record("redis", 5*time.Millisecond)
		return nil
	})
	...
	fiberEnd := tracker.endedAt["fiber"]
	for _, name := range []string{"mongodb", "redis"} {
		if tracker.startedAt[name].Before(fiberEnd) {
