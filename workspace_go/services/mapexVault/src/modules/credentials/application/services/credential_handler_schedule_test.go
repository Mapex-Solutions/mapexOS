...
func TestPublishRefreshSchedule_ExpiredSchedulesImmediately(t *testing.T) {
	mock := &mockScheduleManager{}
	svc := newTestService(mock)
	...
	// An already-expired token is refreshed near-immediately (now + 30s), not dropped —
	// the refresh token is still valid, so this recovers without a user re-login.
	if len(mock.publishedSchedules) != 1 {
		t.Fatalf("expected 1 published schedule for expired token, got %d", len(mock.publishedSchedules))
	}
