...
| Refresh Schedule | Per-credential NATS scheduled message fired at `tokenExpiresAt - 15min` on `VAULT-SCHEDULE` | The reconcile reseed sweep (a single leader-driven pass) |
| Reconciler | Safety-net reseed sweep run by the elected leader on a ticker — reseeds refresh timers missing from `VAULT-SCHEDULE` | The individual refresh consumer |
...
|-------|---------|----------------|-------------|
| Refresh fired | `mapexos.vault.schedule.fired` (stream `VAULT-SCHEDULE`) | `{credentialId, credentialType}` | Self — published by `publishRefreshSchedule` (per-credential re-arm + the reconcile-leader reseed) |
...
- Reconcile leader (KV lease, bucket `LEADER-mapexvault`) — the elected pod runs the reseed sweep (`RunReconcile`) on the `vault_reconcile_interval` ticker, plus an immediate reseed on election; failover is automatic. Replaces the former self-rescheduling reconcile message + `OnMount` arming.
...
- `natsModel.ScheduleManager` (name `core`) — `PublishScheduled`, `PurgeStreamSubject`, `HasPendingMessages` on `VAULT-SCHEDULE`.
- `natsModel.KeyValueStore` (name `vault-leader`) — the `LEADER-mapexvault` lease bucket backing the reconcile leader election.
- `natsModel.LeaderElection` — elects the single pod that runs the reseed sweep (Stop() registered on graceful shutdown).
- Config values `ctx_timeout`, `internal_api_key`, `vault_reconcile_interval`, `nats_kv_replicas`, `service_name`.
...
- Refresh schedules fire at `tokenExpiresAt - 15min` (`RefreshBufferMinutes`); when the target time is already in the past (e.g. the access token expired while the service was down), `publishRefreshSchedule` schedules the refresh near-immediately (`now + 30s`) instead of dropping it — an expired-but-refreshable credential recovers via its refresh token, no user re-login needed.
...
- The reseed sweep runs on exactly one pod (the `LEADER-mapexvault` KV-lease leader) on the `vault_reconcile_interval` ticker; there is no self-republishing message, so no dedup window can stall it and no watchdog is needed for the loop itself.
...
- **NATS infrastructure**: owns the `VAULT-SCHEDULE` stream (file storage + `AllowMsgSchedules`) for the per-credential refresh timers, plus the `LEADER-mapexvault` KV bucket backing the reconcile leader election.
