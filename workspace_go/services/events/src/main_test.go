...
// configuration from main.go: Fiber at P0, ClickHouse/MongoDB/Redis/NATS at P5.
...
func TestShutdownIntegration_MirrorsMainConfig(t *testing.T) {
	...
	// P5: ClickHouse — simulate close (10ms)
	sm.RegisterFunc("clickhouse", 5, func(_ context.Context) error {
		tracker.record("clickhouse", 10*time.Millisecond)
		return nil
	})
	...
	// All 6 hooks must have executed
	if len(order) != 6 {
		t.Fatalf("expected 6 hooks executed, got %d: %v", len(order), order)
	}
	...
	for _, expected := range []string{"clickhouse", "mongodb", "redis-app", "redis-shared", "nats"} {
		if !p5[expected] {
	...
	for _, name := range []string{"clickhouse", "mongodb", "redis-app", "redis-shared", "nats"} {
...
// TestShutdownIntegration_P5ConnectionsConcurrent verifies that ClickHouse,
// MongoDB, Redis and NATS close concurrently (same P5 priority).
func TestShutdownIntegration_P5ConnectionsConcurrent(t *testing.T) {
	...
	// P5: All connections
	sm.RegisterFunc("clickhouse", 5, makeP5Hook("clickhouse"))
	...
	if maxConcurrent.Load() < 5 {
		t.Fatalf("expected all 5 P5 hooks running concurrently, max concurrent was %d", maxConcurrent.Load())
	}
}

// TestShutdownIntegration_ClickHouseErrorDoesNotBlockOthers verifies that
// if ClickHouse Close fails, MongoDB, Redis and NATS still close successfully.
func TestShutdownIntegration_ClickHouseErrorDoesNotBlockOthers(t *testing.T) {
	sm := shutdown.New()

	var mongoClosed atomic.Bool
	...
	sm.RegisterFunc("clickhouse", 5, func(_ context.Context) error {
		return errors.New("clickhouse: broken pipe")
	})
	sm.RegisterFunc("mongodb", 5, func(_ context.Context) error {
		mongoClosed.Store(true)
		return nil
	})
	sm.RegisterFunc("redis-app", 5, func(_ context.Context) error {
	...
	if !mongoClosed.Load() {
		t.Fatal("mongodb should close despite clickhouse error")
	}
	if !redisClosed.Load() {
		t.Fatal("redis should close despite clickhouse error")
	}
	if !natsClosed.Load() {
		t.Fatal("nats should close despite clickhouse error")
	}
}
