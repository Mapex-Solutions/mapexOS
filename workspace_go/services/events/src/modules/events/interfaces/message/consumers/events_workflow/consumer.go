...
/**
 * NewConsumer creates and starts a NATS consumer for workflow execution history events.
 *
...
func NewConsumer(bus *natsModel.Bus, eventService ports.EventServicePort) {
	...
	consumerName := fmt.Sprintf("%s-events-workflow", serviceName)
	queueGroup := fmt.Sprintf("%s-EVENTS-WORKFLOW-GROUP", serviceName)

	natsBatchSize, _ := config.GetIntValue("nats_batch_size")
	natsFetchTimeout, _ := config.GetIntValue("nats_fetch_timeout")

	logger.Info(fmt.Sprintf("[CONSUMER:EventsWorkflow] Starting %s with retry/DLQ support", consumerName))
	...
	_, err := bus.StartConsumer(natsModel.ConsumerOptions{
		...
		BatchMessageHandlerV2: func(messages []*natsModel.Message) {
			eventService.ProcessWorkflowEventBatch(messages)
		},
	})

	if err != nil {
		logger.Error(err, "[CONSUMER:EventsWorkflow] Failed to start consumer")
		return
	}

	logger.Info("[CONSUMER:EventsWorkflow] Started successfully")
}
