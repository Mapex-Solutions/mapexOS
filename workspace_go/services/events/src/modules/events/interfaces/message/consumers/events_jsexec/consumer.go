...
/**
 * NewConsumer creates and starts a NATS consumer for JS Executor debug events.
 *
 ...
 *
 * This consumer subscribes to the "events.logs.jsexecutor" subject on the "EVENTS-JSEXEC" stream
 ...
 *   - eventService: The EventServicePort interface for processing JS exec events
 *
...
func NewConsumer(bus *natsModel.Bus, eventService ports.EventServicePort) {
	...
	consumerName := fmt.Sprintf("%s-events-jsexec", serviceName)
	queueGroup := fmt.Sprintf("%s-EVENTS-JSEXEC-GROUP", serviceName)

	natsBatchSize, _ := config.GetIntValue("nats_batch_size")
	natsFetchTimeout, _ := config.GetIntValue("nats_fetch_timeout")

	logger.Info(fmt.Sprintf("[CONSUMER:EventsJsExec] Starting %s with retry/DLQ support", consumerName))
	...
	_, err := bus.StartConsumer(natsModel.ConsumerOptions{
		...
		BatchMessageHandlerV2: func(messages []*natsModel.Message) {
			eventService.ProcessJsExecEventBatch(messages)
		},
	})

	if err != nil {
		logger.Error(err, "[CONSUMER:EventsJsExec] Failed to start consumer")
		return
	}

	logger.Info("[CONSUMER:EventsJsExec] Started successfully with retry/DLQ support")
}
