...
/**
 * NewConsumer creates and starts a NATS consumer for business rule execution history events.
 *
 ...
 *
 * This consumer subscribes to the "events.businessrule" subject on the "EVENTS-BUSINESSRULE" stream
 ...
 *   - eventService: The EventServicePort interface for processing business rule events
 *
...
func NewConsumer(bus *natsModel.Bus, eventService ports.EventServicePort) {
	...
	consumerName := fmt.Sprintf("%s-events-businessrule", serviceName)
	queueGroup := fmt.Sprintf("%s-EVENTS-BUSINESSRULE-GROUP", serviceName)

	natsBatchSize, _ := config.GetIntValue("nats_batch_size")
	natsFetchTimeout, _ := config.GetIntValue("nats_fetch_timeout")

	logger.Info(fmt.Sprintf("[CONSUMER:EventsBusinessRule] Starting %s with retry/DLQ support", consumerName))
	...
	_, err := bus.StartConsumer(natsModel.ConsumerOptions{
		...
		BatchMessageHandlerV2: func(messages []*natsModel.Message) {
			eventService.ProcessBusinessRuleEventBatch(messages)
		},
	})

	if err != nil {
		logger.Error(err, "[CONSUMER:EventsBusinessRule] Failed to start consumer")
		return
	}

	logger.Info("[CONSUMER:EventsBusinessRule] Started successfully with retry/DLQ support")
}
