# Bounded Context: App (Events Bootstrap)
...
This is NOT a business bounded context — it is the **service bootstrap orchestrator** for the `events` service. Its single responsibility is to iterate the module registry declared in `src/shared/configuration/modules/config.go` and invoke each module's lifecycle hooks (`InitRepositories`, `InitServices`, `InitInterfaces`) in the correct order. It carries no domain logic, no ports, no entities, and no state of its own — it only wires the DI container and starts the consumer/HTTP surfaces of the real modules (`events`, `retention`, `asset_status`).
...
| Lifecycle phase | One of the three ordered init passes: Repositories → Services → Interfaces | Service runtime phases (ingest, batch, archive, etc.) |
...
- No domain types, no ports, no DI struct — the module MUST remain a thin orchestration loop.
...
- Calls lifecycle hooks of the real bounded contexts in this service: **events** (the main event ingest/query module), **retention** (policy + TTL manager), **asset_status** (asset connectivity timeseries).
