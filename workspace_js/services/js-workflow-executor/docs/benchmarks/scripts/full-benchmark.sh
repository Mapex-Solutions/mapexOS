...
#
# Architecture:
#   CPU_LIMIT=N → piscinaWorkers = N-1, batchSize = N × 500
#   Each worker: own V8 Isolate + scriptCache
#
# Flow:
#   — Preflight checks (infra health + user confirmation)
#   — Seed MinIO (workflow script source) — once before all tests
#   — For each CPU config:
#      a. Purge NATS streams
#      b. Populate stream with N messages
#      c. Start js-workflow-executor in isolated cgroup
#      d. Monitor drain until complete
#      e. Capture Prometheus metrics
#      f. Stop js-workflow-executor
#   — Teardown MinIO
#
# Prerequisites:
...
wait_for_stream_populate() {
    ...
    echo "  ║                                                             ║"
    echo "  ║  Teste:    CPU_LIMIT=$cpu ($(( cpu > 1 ? cpu - 1 : 1 )) workers)"
    echo "  ║  Stream:   $STREAM"
...
# Verify cgroup shield exists
verify_shield || exit 1

# Validate CPU list doesn't exceed shield capacity
validate_cpu_list "$CPU_LIST"

# Validate SERVICE_DIR exists
if [ ! -f "$SERVICE_DIR/src/main.ts" ]; then
    log_error "Service not found at $SERVICE_DIR/src/main.ts"
    echo "Expected: workspace_js/services/js-workflow-executor/src/main.ts"
    exit 1
fi
...
echo ""
log_info "Seeding workflow script source to MinIO..."
...
echo ""
echo "================================================================"
echo "  JS-Workflow-Executor Piscina Benchmark (cgroup v2 isolated)"
echo "================================================================"
...
for CPU in $CPU_LIST; do
    ...
    bench_seed_messages "$EVENT_COUNT" "$SEED_DIR/nats/workflow-code-execute.json"
    ...
    # Start js-workflow-executor in isolated cgroup
    log_step "3" "6" "Starting js-workflow-executor (CPU_LIMIT=$CPU, workers=$WORKERS, cores $CORE_RANGE)..."
    ...
    # Capture metrics (wait a few seconds for final metrics flush)
    ...
    # Latency
    LATENCY_AVG=$(extract_histogram_avg "$METRICS" "wfexec_event_duration_seconds" 'consumer="workflow_js_code"')

    # Events
    EVENTS_SUCCESS=$(extract_metric "$METRICS" "wfexec_events_processed_total" 'status="success"')
    EVENTS_FAILED=$(extract_metric "$METRICS" "wfexec_events_processed_total" 'status="error"')
    SCRIPT_ERRORS=$(extract_metric "$METRICS" "wfexec_script_errors_total")
    OOM_COUNT=$(extract_metric "$METRICS" "wfexec_pool_recycled_total" 'reason="oom"')
    ...
    EVT_COUNT=$(echo "$METRICS" | grep '^wfexec_event_duration_seconds_count{.*workflow_js_code' 2>/dev/null | head -1 | awk '{print $NF}') || true
    EVT_COUNT="${EVT_COUNT:-0}"
    HALF_COUNT=$(echo "$EVT_COUNT" | awk '{printf "%.0f", $1*0.5}')
    P50_BUCKET=$(echo "$METRICS" | grep '^wfexec_event_duration_seconds_bucket{.*workflow_js_code' 2>/dev/null | \
    ...
    P99_BUCKET=$(echo "$METRICS" | grep '^wfexec_event_duration_seconds_bucket{.*workflow_js_code' 2>/dev/null | \
        awk -v target="$P99_TARGET" '{val=$NF; if(val+0 >= target+0 && !found){gsub(/.*le="/,""); gsub(/".*/,""); printf "%.1f", $0*1000; found=1}}') || true

    # Stop js-workflow-executor
    log_step "6" "6" "Stopping js-workflow-executor..."
    kill "$JS_PID" 2>/dev/null || true
    wait "$JS_PID" 2>/dev/null || true
    sleep 2
