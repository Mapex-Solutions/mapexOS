// Package iam_bootstrap is the shared IAM bootstrap building block every saga
// journey composes. It lives under common/journey/ — NOT inside any single
// journey — because a journey that imported another journey's phase to reuse it
// would couple the two; shared journey pieces belong in common.
//
// The bootstrap authenticates as the seed admin user (admin@mapex.local) and
// preconfigures every per-service HTTP client in the saga ClientSet with
// the resulting bearer plus the seed root org as X-Org-Context. After it
// finishes, every downstream item can drive any service endpoint as the
// seeded admin without re-issuing logins or maintaining its own context.
//
// It leaves the bag populated with:
//
//	iam.userJWT             bearer attached to every HTTP client
//	iam.organizationID      MapexosOrgID — the seed root used as saga org
//
// It does NOT create a new tenant org, role, or user. The seed admin already
// carries the wildcard "mapex.*" permission anchored at the seed root
// (MapexosOrgID); reusing that anchor keeps each journey focused on its own
// flow and avoids the permission-cache rebuild path that needs work on the
// mapexIam side before brand-new child orgs can be driven by the bootstrap
// actor end to end. Tenant-isolation tests belong in their own journey
// (planned: iot/multi_tenant_isolation/).
//
...
//
// Outcome (what passing this bootstrap proves):
...
//     X-Org-Context propagated; downstream items need not touch headers.
//
...
func BootstrapItems() []saga.Item {
	return []saga.Item{
		// Seed admin login (admin@mapex.local) via /auth/login; publishes JWT + organizationID to the bag.
		authSteps.SeedAdminLogin(),

		// JWT validity check: token parses, signature matches, expiration is in the future.
		authAsserts.AssertJwtValid(),

		// Coverage check: JWT carries access to MapexosOrgID so downstream phases can call any service.
