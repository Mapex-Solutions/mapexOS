...
_run_tests_completion() {
    ...
    # Top-level commands
    opts="all check list help saga -q --quiet -p --parallel -t --timeout -h --help"
    ...
    # Saga trigger journey targets
    saga_targets="triggers trigger-http trigger-email trigger-slack trigger-teams trigger-mqtt trigger-nats trigger-rabbitmq trigger-websocket"
    ...
    case "${prev}" in
        ...
        http_gateway)
            ...
            ;;
        saga)
            COMPREPLY=( $(compgen -W "${saga_targets}" -- ${cur}) )
            return 0
            ;;
        -p|--parallel)
