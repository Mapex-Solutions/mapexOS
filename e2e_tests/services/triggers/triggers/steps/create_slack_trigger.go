...
func CreateSlackTrigger() saga.Step {
	return saga.Step{
		...
		Do: func(c *saga.Context) error {
			...
			if resp.StatusCode < 200 || resp.StatusCode >= 300 {
				...
				return fmt.Errorf("create slack trigger: unexpected status %d body=%s", resp.StatusCode, string(body))
			}
			var out triggerCreateResponse
			if err := json.NewDecoder(resp.Body).Decode(&out); err != nil {
				return fmt.Errorf("decode create slack trigger response: %w", err)
			}
			if out.Data.ID == "" {
				return fmt.Errorf("create slack trigger: empty id in response")
			}
