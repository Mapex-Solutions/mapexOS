...
// SagaLorawanDecodedTemplate returns a LoRaWAN sensor template whose first script
// (the codec) is the REAL Dragino LHT65N-S-DS decoder from the mapexMarketplace
// catalog. Unlike the pass-through SagaLorawanSensorTemplate (which keeps the raw
// bytes in data), this ScriptConversion runs the device codec: it turns the uplink
// bytes into the device's semantic fields (TempC_SHT, Hum_SHT, BatV, ...) inside the
// StandardizedPayload's data, so the journey can assert the DECODED output — the
// codec fundamental that the raw-ingest journeys never exercise.
func SagaLorawanDecodedTemplate(runID string) *AssetTemplateCreateBuilder {
	return &AssetTemplateCreateBuilder{
		spec: contracts.AssetTemplateCreate{
			Name:             fmt.Sprintf("saga-lorawan-decoded-%s", runID),
			Description:      zerovalue.Ptr("Saga LoRaWAN decoded template (real Dragino LHT65N codec)"),
			Enabled:          true,
			AssetIDPath:      "assetUUID",
			ScriptConversion: lht65nScriptConversion,
		},
	}
}

// lht65nScriptConversion is the template's first script: the real Dragino
// LHT65N-S-DS decodeUplink (sourced verbatim from the mapexMarketplace catalog,
// vendors/dragino/lht65n-s-ds/codecs/dragino-generic/decoder.js), followed by a
// wrapper that maps the decoded output into the platform StandardizedPayload
// (eventType/eventId/data/created). js-executor hands the script `payload`
// (`{ bytes, fPort, fCnt, rxInfo }`) and reads back `result`.
const lht65nScriptConversion = `
...
const __decoded = decodeUplink({ bytes: payload.bytes, fPort: payload.fPort });
const result = {
  eventType: 'lorawan',
  eventId: String(payload.fCnt),
  data: (__decoded && __decoded.data) ? __decoded.data : {},
