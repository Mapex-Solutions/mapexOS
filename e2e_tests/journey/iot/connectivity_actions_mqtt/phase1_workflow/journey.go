...
func Items() []saga.Item {
	return []saga.Item{
		...
		// MQTT connectivity asset wired to the two workflow-kind route groups.
		assetSteps.CreateConnectivityAsset(mqttConnectivityAsset()),

		// Warm-up CONNECT: first-ever observation is unknown→online and is silent by design (no workflow fires).
		...
		// Real transition 1: DISCONNECT drives online→offline; offline RG matches → workflow fires.
		assetSteps.DisconnectMqtt(),

		// Confirm the healthmonitor saw the transition.
		assetAsserts.AssertHealthStatusEventually("offline"),

		// Smoke oracle: events service surfaces a workflow execution scoped after the disconnect timestamp.
		eventAsserts.AssertWorkflowEventReceivedAfter(assetSteps.BagKeyMqttDisconnectedAt),

		// Real transition 2: a fresh CONNECT drives offline→online; online RG matches → workflow fires again.
		assetSteps.ConnectMqttPassword(),

		// Confirm the asset is online again.
		assetAsserts.AssertHealthStatusEventually("online"),

		// Smoke oracle: events service surfaces a second workflow execution scoped after the connect timestamp.
