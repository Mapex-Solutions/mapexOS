# Phase 2: Event pipeline (HTTP trigger)

## What this test proves

HTTP trigger fires from the full telemetry pipeline: a POST to the
gateway is transformed by the js-executor against the asset's
template script, routed by the trigger router, and the HTTP trigger
executor POSTs to an in-process HTTP sink.

This is the slice phase1 does not cover — phase1 drives health
transitions; phase2 drives a real `POST /api/v1/events` and exercises
the StandardizedPayload transform sitting between the gateway and
the router.
...
- The js-executor runs the template script and emits a standardized
  payload.
- The route group's trigger router selects the HTTP trigger.
- `events_trigger` records at least one successful execution.
- The in-process HTTP sink receives the POST the trigger emitted.
...
- Trigger config wrong — HTTP sink not reachable, no POST observed.
...
HTTP sink, trigger, route groups, data source, template, and
