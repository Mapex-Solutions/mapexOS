// Package phase2_event_pipeline exercises the Teams trigger via the
// telemetry path: POST /api/v1/events → gateway → js-executor →
// router → trigger → HTTP sink (Teams webhooks are POST HTTP).
...
func Items() []saga.Item {
	return []saga.Item{
		// Boot the in-process HTTP sink (Teams webhook = POST HTTP).
		triggerSteps.StartTestSink(),

		// Create a Teams trigger whose webhookUrl points at the HTTP sink.
		...
		// Content-key: the HTTP sink received the Teams webhook POST.
