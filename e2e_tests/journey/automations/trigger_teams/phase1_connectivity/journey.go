// Package phase1_connectivity exercises the Teams trigger smoke
// path via the connectivity pipeline. The Teams executor POSTs the
// webhook URL as a real HTTP request, so the saga reuses the
// in-process HTTP sink (StartTestSink) instead of standing up a
// dedicated Teams-shaped server.
...
func Items() []saga.Item {
	return []saga.Item{
		// Boot the shared HTTP sink (Teams webhooks are POST HTTP under the hood).
		triggerSteps.StartTestSink(),

		// Create a Teams-kind trigger whose webhookUrl points at the HTTP sink.
		triggerSteps.CreateTeamsTrigger(),

		// Route group for online health transitions → Teams trigger.
		rgSteps.CreateRouteGroupAt(rgSteps.BagKeyOnlineRouteGroupID, onlineTriggerRG()),

		// Route group for offline health transitions → Teams trigger.
		...
		// Force offline → offline RG fires Teams trigger → POSTs to HTTP sink.
		assetSteps.ForceOfflineByAdmin("saga-teams-phase1-warmup"),

		// Confirm offline observed.
		assetAsserts.AssertHealthStatusEventually("offline"),

		// Smoke: sink received the Teams webhook POST.
		triggerAsserts.AssertSinkHitEventually(1),

		// Heartbeat brings asset back online; online RG fires Teams trigger again.
		assetSteps.SendHttpHeartbeat(),

		// Confirm online observed again.
		assetAsserts.AssertHealthStatusEventually("online"),

		// Smoke: sink received the second Teams POST.
