// Package phase2_event_pipeline exercises the Slack trigger via the
// telemetry path: POST /api/v1/events → gateway → js-executor →
// router → trigger → HTTP sink (Slack webhooks are POST HTTP).
...
func Items() []saga.Item {
	return []saga.Item{
		// Boot the in-process HTTP sink (Slack webhook = POST HTTP).
		triggerSteps.StartTestSink(),

		// Create a Slack trigger whose webhookUrl points at the HTTP sink.
		...
		// Content-key: the HTTP sink received the Slack webhook POST.
