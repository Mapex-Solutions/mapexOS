# Fase 2: Pipeline de eventos (trigger Slack)

## O que este teste prova

Trigger Slack dispara pelo pipeline completo de telemetria: um POST
...
Slack entrega o webhook (webhooks Slack são POST HTTP) para um sink
HTTP in-process que substitui `hooks.slack.com`.
...
- O js-executor executa o script do template e emite um payload
  padronizado.
- O router de trigger do route group seleciona o trigger Slack.
- `events_trigger` registra ao menos uma execução bem-sucedida.
- O sink HTTP in-process recebe o POST do webhook Slack.
...
- `webhookUrl` do trigger Slack desviou do sink HTTP gerenciado pela
...
próprio sink HTTP, trigger Slack, route groups, data source,
