# Phase 2: Event pipeline (Slack trigger)

## What this test proves

Slack trigger fires from the full telemetry pipeline: a POST to the
gateway is transformed by the js-executor against the asset's
template script, routed by the trigger router, and the Slack trigger
executor delivers the webhook (Slack webhooks are POST HTTP) to an
in-process HTTP sink standing in for `hooks.slack.com`.
...
- The js-executor runs the template script and emits a standardized
  payload.
- The route group's trigger router selects the Slack trigger.
- `events_trigger` records at least one successful execution.
- The in-process HTTP sink receives the Slack webhook POST.
...
- Slack trigger `webhookUrl` drifts from the saga-managed HTTP sink,
  or the sink is not reachable from the triggers service.

## How to run

```bash
cd e2e_tests
go test -tags=saga -v ./journey/automations/trigger_slack/phase2_event_pipeline
```
...
HTTP sink, Slack trigger, route groups, data source, template, and
