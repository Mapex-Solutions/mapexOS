// Package phase2_event_pipeline exercises the RabbitMQ trigger via
// the telemetry path: POST /api/v1/events → gateway → js-executor →
// router → trigger → ephemeral RabbitMQ container.
...
func Items() []saga.Item {
	return []saga.Item{
		// Spin up an ephemeral RabbitMQ container via testcontainers-go.
		triggerSteps.StartRabbitmqContainer(),

		// Create a RabbitMQ trigger pointing at the saga-managed container.
		...
		// Content-key: resolved config carries the saga-scoped queue.
