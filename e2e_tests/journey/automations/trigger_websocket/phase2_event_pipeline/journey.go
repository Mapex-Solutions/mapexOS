// Package phase2_event_pipeline exercises the WebSocket trigger via
// the telemetry path: POST /api/v1/events → gateway → js-executor →
// router → trigger → WS sink.
...
func Items() []saga.Item {
	return []saga.Item{
		// Boot the in-process WS sink the trigger executor will dial.
		triggerSteps.StartWebsocketSink(),

		// Create a WebSocket trigger pointing at the WS sink.
		...
		// Content-key: resolved config carries the saga message.
