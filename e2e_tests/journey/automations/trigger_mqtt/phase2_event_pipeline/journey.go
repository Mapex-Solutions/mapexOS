// Package phase2_event_pipeline exercises the MQTT trigger via the
// telemetry path: POST /api/v1/events → gateway → js-executor →
// router → trigger → in-process MQTT broker.
...
func Items() []saga.Item {
	return []saga.Item{
		// Boot an in-process MQTT broker (mochi-mqtt) on a free ephemeral port.
		triggerSteps.StartMqttBroker(),

		// Create an MQTT trigger pointing at the saga-managed broker.
		...
		// Content-key: resolved config carries the saga-scoped topic.
		eventAsserts.AssertLastTriggerRequestDataContains("mapex-saga/mqtt/"),
