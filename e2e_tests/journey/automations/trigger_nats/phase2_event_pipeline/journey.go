// Package phase2_event_pipeline exercises the NATS trigger via the
// telemetry path: POST /api/v1/events → gateway → js-executor →
// router → trigger → embedded NATS server.
...
func Items() []saga.Item {
	return []saga.Item{
		// Boot an embedded NATS server (nats-server/v2) on a free ephemeral port.
		triggerSteps.StartNatsServer(),

		// Create a NATS trigger pointing at the saga-managed server.
		triggerSteps.CreateNatsTrigger(),

		// Single trigger-kind RG reused for online/offline/general.
		...
		// POST a telemetry event to the gateway.
		...
		// Content-key: resolved config carries the saga-scoped subject.
