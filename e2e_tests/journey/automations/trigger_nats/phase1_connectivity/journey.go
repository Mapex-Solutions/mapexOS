// Package phase1_connectivity exercises the NATS trigger smoke
// path via the connectivity pipeline. Uses the events_trigger
// oracle (success=true is set after the NATS publish call returns
// nil) instead of a per-broker subscriber.
...
func Items() []saga.Item {
	return []saga.Item{
		// Boot an embedded NATS server (nats-server/v2) on a free ephemeral port, no auth.
		triggerSteps.StartNatsServer(),

		// Create a NATS trigger pointing at the saga-managed server (URL read from bag).
		triggerSteps.CreateNatsTrigger(),

		// Route group for online health transitions → NATS trigger.
		rgSteps.CreateRouteGroupAt(rgSteps.BagKeyOnlineRouteGroupID, onlineTriggerRG()),

		// Route group for offline health transitions → NATS trigger.
		...
		// Force offline → offline RG fires NATS trigger → publishes to server.
		assetSteps.ForceOfflineByAdmin("saga-nats-phase1-warmup"),
		...
		// Content-key: resolved config carries the saga-scoped NATS subject.
		eventAsserts.AssertLastTriggerRequestDataContains("mapex.saga.nats."),

		// Heartbeat brings asset back online; online RG fires NATS trigger again.
		assetSteps.SendHttpHeartbeat(),
