# Fase 2: Pipeline de eventos (trigger Email)

## O que este teste prova

Trigger Email dispara pelo pipeline completo de telemetria: um POST
...
Email entrega para um sink SMTP in-process.
...
- O js-executor executa o script do template e emite um payload
  padronizado.
- O router de trigger do route group seleciona o trigger Email.
- `events_trigger` registra ao menos uma execução bem-sucedida.
- O sink SMTP in-process captura a entrega a partir do `from` / `to`
  do envelope.
...
- Sink SMTP não associado, ou configuração Email do trigger (host /
  porta / credenciais) desviou do sink gerenciado pela saga.
...
próprio sink SMTP, trigger Email, route groups, data source,
